...
```bash
python examples/demo_3x3_layered.py --config configs/demo_3x3.json
```

The demo calls the same conversion logic, then writes explanatory PNG figures
to `outputs/demo_3x3/`. It is useful for inspecting a small system but is not
the formal production workflow.
